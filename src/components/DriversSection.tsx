...
const DriversSection = () => {
  ...
  const driversProducts = products.filter(product => product.tags?.includes('Drivers') && product.is_active !== false).slice(0, 4);
  useEffect(() => {
    const fetchSalesData = async () => {
      try {
        ...
        const updatedProducts = driversProducts.map(product => {
        ...
        setProductsWithSales(driversProducts);
      }
    };
    fetchSalesData();
  }, [products]);
  ...
  return <section className="py-8 lg:py-16 bg-white">
      <div className="container mx-auto px-4">
        <h2 className="text-2xl text-center mb-8 lg:mb-12 lg:text-2xl font-medium text-zinc-950">
          DRIVERS
        </h2>
            ...
            return <div key={product.id} className="group cursor-pointer bg-white" onClick={() => handleProductClick(product)}>
                  ...
                  backgroundColor: '#C24242'
                    ...
                    <div className="text-left space-y-1">
                      <h3 className="text-sm lg:text-base font-normal text-black leading-tight">
                        {product.name}
                      </h3>
                      <div className="text-sm lg:text-base font-normal text-black">
                        {isOnSale ? <div className="flex flex-col lg:flex-row lg:items-center lg:gap-2">
                            <span className="line-through text-zinc-600 font-normal lg:text-sm text-sm">Tk {product.originalPrice}.00</span>
