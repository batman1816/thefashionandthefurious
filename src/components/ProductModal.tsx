        ...
        <div className="grid grid-cols-1 lg:grid-cols-2">
          ...
          <div className="p-12 flex flex-col">
            ...
            {availableSizes.length > 0 && <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2 max-w-full overflow-x-auto">
                  {availableSizes.map(size => <button key={size} onClick={() => handleSizeSelect(size)} className={`flex-shrink-0 px-4 py-2 text-sm font-light transition-all duration-200 border-2 border-black rounded-full ${selectedSize === size ? 'bg-black text-white' : 'bg-white text-black hover:bg-gray-50'}`} style={{fontFamily: 'Poppins', fontWeight: 300}}>
                      {size}
                    </button>)}
                </div>
              </div>}
