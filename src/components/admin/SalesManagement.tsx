...
const SalesManagement = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const {
        data: bundleData,
        error: bundleError
      } = await supabase.from('bundle_deals').select('*').order('created_at', {
        ascending: false
      });
      if (bundleError) {
      ...
      const {
        data: productsData,
        error: productsError
      } = await supabase.from('products').select('id, name, price, category, image_url').eq('is_active', true);
      if (productsError) {
  ...
  const handleSaveSale = async () => {
    ...
    try {
      ...
      if (editingSale) {
        const {
          error
        } = await supabase.from('sales').update(formData).eq('id', editingSale.id);
        if (error) {
        ...
        const {
          error
        } = await supabase.from('sales').insert([formData]);
        if (error) {
  ...
  const handleSaveBundle = async () => {
    ...
    try {
      ...
      if (editingBundle) {
        const {
          error
        } = await supabase.from('bundle_deals').update(formData).eq('id', editingBundle.id);
        if (error) {
        ...
        const {
          error
        } = await supabase.from('bundle_deals').insert([formData]);
        if (error) {
  ...
  return <div className="text-white">
      ...
      <Card className="mb-8 bg-zinc-800 border-gray-700">
        ...
        <CardContent>
          {sales.length === 0 ? <div className="text-center py-8 text-gray-400">
              No product sales created yet. Create your first sale to get started!
            </div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {sales.map(sale => <div key={sale.id} className="bg-zinc-900 p-4 rounded-lg border border-zinc-700">
      ...
      <Card className="mb-8 bg-zinc-800 border-gray-700">
        ...
        <CardContent>
          {bundleDeals.length === 0 ? <div className="text-center py-8 text-gray-400">
              No bundle deals created yet. Create your first bundle deal to get started!
            </div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {bundleDeals.map(bundle => <div key={bundle.id} className="bg-zinc-900 p-4 rounded-lg border border-zinc-700">
                  <div className="flex justify-between items-start mb-3">
                ...
                setShowBundleForm(true);
                    ...
                    <Button size="sm" variant="destructive" onClick={() => handleDeleteBundle(bundle.id)}>
                      <Trash2 size={14} />
                    </Button>
                  </div>
                </div>)}
      ...
      {/* Sale Form Modal */}
      {showSaleForm && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-zinc-800 p-6 rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
            ...
            }} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded text-white" required>
              ...
              <div>
                ...
                <input type="text" value={saleForm.sale_title} onChange={e => setSaleForm({
              ...saleForm,
              sale_title: e.target.value
            })} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded text-white" placeholder="e.g., Summer Sale" required />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Sale Description</label>
                <textarea value={saleForm.sale_description} onChange={e => setSaleForm({
              ...saleForm,
              sale_description: e.target.value
            })} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded text-white" rows={3} placeholder="Describe the sale..." />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Original Price *</label>
                  <input type="number" value={saleForm.original_price} onChange={e => setSaleForm({
                ...saleForm,
                original_price: parseInt(e.target.value) || 0
              })} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded text-white" min="0" required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Sale Price *</label>
                  <input type="number" value={saleForm.sale_price} onChange={e => setSaleForm({
                ...saleForm,
                sale_price: parseInt(e.target.value) || 0
              })} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded text-white" min="0" max={saleForm.original_price - 1} required />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="datetime-local" value={saleForm.start_date} onChange={e => setSaleForm({
                ...saleForm,
                ...
                <div>
                  ...
                  <input type="datetime-local" value={saleForm.end_date} onChange={e => setSaleForm({
                ...saleForm,
              ...
              <div className="flex items-center">
                <input type="checkbox" checked={saleForm.is_active} onChange={e => setSaleForm({
              ...saleForm,
      ...
      {/* Bundle Form Modal */}
      {showBundleForm && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-zinc-800 p-6 rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-bold text-white mb-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={bundleForm.name} onChange={e => setBundleForm({
              ...bundleForm,
              name: e.target.value
            })} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded text-white" placeholder="e.g., Buy 2 Get 1 Half Off" required />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Description</label>
                <textarea value={bundleForm.description} onChange={e => setBundleForm({
              ...bundleForm,
              description: e.target.value
            })} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded text-white" rows={3} placeholder="Describe the bundle deal..." />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Minimum Quantity *</label>
                  <input type="number" value={bundleForm.minimum_quantity} onChange={e => setBundleForm({
                ...bundleForm,
                minimum_quantity: parseInt(e.target.value) || 2
              })} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded text-white" min="2" required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Discount % *</label>
                  <input type="number" value={bundleForm.discount_percentage} onChange={e => setBundleForm({
                ...bundleForm,
                discount_percentage: parseInt(e.target.value) || 50
              })} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded text-white" min="1" max="99" required />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Max Discount Items *</label>
                <input type="number" value={bundleForm.max_discount_items} onChange={e => setBundleForm({
              ...bundleForm,
              max_discount_items: parseInt(e.target.value) || 1
            })} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded text-white" min="1" required />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Start Date (Optional)</label>
                  <input type="datetime-local" value={bundleForm.start_date} onChange={e => setBundleForm({
                ...bundleForm,
                start_date: e.target.value
              })} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded text-white" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">End Date (Optional)</label>
                  <input type="datetime-local" value={bundleForm.end_date} onChange={e => setBundleForm({
                ...bundleForm,
                end_date: e.target.value
              })} className="w-full px-3 py-2 bg-zinc-700 border border-zinc-600 rounded text-white" />
                </div>
              ...
              <div className="flex items-center">
                <input type="checkbox" checked={bundleForm.is_active} onChange={e => setBundleForm({
              ...bundleForm,
              is_active: e.target.checked
            })} className="mr-2" />
                <label className="text-sm text-gray-300">Active</label>
