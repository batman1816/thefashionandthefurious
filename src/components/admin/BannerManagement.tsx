...
const BannerManagement = () => {
  ...
  return <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {!isAddingBanner && <button onClick={() => setIsAddingBanner(true)} className="text-white px-4 py-2 rounded flex items-center gap-2 bg-zinc-800 hover:bg-zinc-700">
            <Plus size={20} />
            Add Banner
          </button>}
      </div>

      {isAddingBanner && <div className="bg-gray-800 p-6 rounded-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="space-y-4">
                <div className="border-2 border-dashed border-gray-600 rounded-lg p-4">
                  ...
                  <label htmlFor="banner-upload" className={`cursor-pointer flex flex-col items-center justify-center text-gray-400 hover:text-white ${uploading ? 'opacity-50' : ''}`}>
                ...
                {formData.image_url && <div className="mt-4">
                    <img src={formData.image_url} alt="Preview" className="w-full max-w-md h-32 object-cover rounded border-2 border-gray-600" />
                  </div>}
              </div>
            </div>
            ...
            }))} className="w-full px-3 py-2 bg-gray-700 text-white rounded" placeholder="e.g., Shop Now" />
              </div>
              <div>
                <label className="block text-white mb-2">Button Link (Optional)</label>
                <input type="text" value={formData.button_link} onChange={e => setFormData(prev => ({
              ...prev,
              button_link: e.target.value
            }))} className="w-full px-3 py-2 bg-gray-700 text-white rounded" placeholder="e.g., /drivers" />
              </div>
            </div>
            ...
            <div className="flex gap-4 pt-4">
              <button type="submit" disabled={uploading} className="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded disabled:opacity-50">
            ...
            <tbody>
              {banners.map(banner => <tr key={banner.id} className="border-b border-gray-700">
                  <td className="px-6 py-4">
                    <img src={banner.image_url} alt="Banner" className="w-24 h-12 object-cover rounded" />
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      <button onClick={() => toggleBannerStatus(banner)} className={`p-2 rounded ${banner.is_active ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-green-600 hover:bg-green-700'} text-white`}>
                        {banner.is_active ? <EyeOff size={16} /> : <Eye size={16} />}
                      </button>
                      <button onClick={() => handleEdit(banner)} className="bg-blue-600 hover:bg-blue-700 text-white p-2 rounded">
                        <Edit size={16} />
                      </button>
                      <button onClick={() => handleDelete(banner)} className="bg-red-600 hover:bg-red-700 text-white p-2 rounded">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
