...
const MousepadManagement = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const productData = {
      ...
      price: Object.keys(formData.size_pricing).length > 0 ? Math.min(...Object.values(formData.size_pricing).filter(p => p > 0)) : parseInt(formData.price || '0'),
      category: 'mousepads',
  ...
  return <div className="space-y-6">
      ...
      {isAddingProduct && <div className="p-6 rounded-lg bg-zinc-800">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            }))} className="w-full px-3 py-2 text-white rounded bg-zinc-900" placeholder="Optional - will use size pricing if set" />
              </div>
            </div>
          ...
          }))} placeholder="Enter product description. Use line breaks for formatting." className="w-full px-3 py-2 text-white rounded h-32 bg-zinc-900" />
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                 {MOUSEPAD_SIZES.map(size => <div key={size} className="flex items-center gap-4">
                     <label className="flex items-center space-x-2 text-white min-w-[150px]">
                       <input type="checkbox" checked={formData.sizes.includes(size)} onChange={e => {
                  if (e.target.checked) {
                    setFormData(prev => ({
                      ...prev,
                      ...
                      size_pricing: {
                        ...prev.size_pricing,
                        [size]: size === '900 X 400 MM' ? 2499 : size === '700 X 300 MM' ? 1999 : size === '350 X 300 MM' ? 1499 : 0
                      }
                    }));
                  } else {
                    setFormData(prev => ({
                     ...
                     {formData.sizes.includes(size) && <input type="number" placeholder="Price in Taka" value={formData.size_pricing[size] || ''} onChange={e => handleSizePricingChange(size, e.target.value)} className="px-3 py-2 text-white rounded bg-zinc-900 w-32" />}
                   </div>)}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-4">
                {formData.sizes.map(size => <div key={size} className="border border-gray-600 rounded p-4">
                    ...
                    try {
                      ...
                      setFormData(prev => ({
                        ...prev,
                        size_variants: [...prev.size_variants.filter(sv => sv.size !== size), {
                          size,
                          image_url: imageUrl
                        }]
                      }));
                      ...
                      {formData.size_variants.find(sv => sv.size === size) && <div>
                          <label className="block text-white mb-2">Current Image</label>
                          <img src={formData.size_variants.find(sv => sv.size === size)?.image_url} alt={`${size} variant`} className="w-full h-20 object-cover rounded" />
                        </div>}
                    </div>
                  </div>)}
              </div>
      ...
      <div className="bg-gray-800 rounded-lg overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-700">
              <tr>
                <th className="py-3 text-left text-white bg-zinc-800 px-[24px]">Image</th>
            ...
            <tbody>
              {mousepadProducts.map(product => <tr key={product.id} className="border-b border-gray-700">
                  ...
                  <td className="text-white px-[24px] py-[16px] bg-zinc-800">
                    {product.size_pricing ? `From Tk${Math.min(...Object.values(product.size_pricing).filter((p: number) => p > 0))}` : `Tk${product.price}`}
                  </td>
                  <td className="text-white px-[24px] py-[16px] bg-zinc-800">{product.sizes.join(', ')}</td>
                  <td className="px-[24px] py-[16px] bg-zinc-800">
                    <Switch checked={product.is_active} onCheckedChange={() => toggleProductStatus(product)} className="bg-green-500 hover:bg-green-400 text-zinc-50 text-base" />
                  </td>
                  <td className="px-[32px] py-[16px] bg-zinc-800">
                    <div className="flex gap-2">
                      <button onClick={() => handleEdit(product)} className="text-blue-400 hover:text-blue-300">
                        <Edit size={16} className="px-0 py-0 mx-0 bg-transparent" />
                      </button>
        ...
        {mousepadProducts.length === 0 && <div className="text-center py-8 text-gray-400">
