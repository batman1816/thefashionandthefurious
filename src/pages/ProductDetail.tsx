        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col">
            ...
            {availableSizes.length > 0 && <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {availableSizes.map(size => <button key={size} onClick={() => handleSizeSelect(size)} className={`px-4 py-2 md:px-3 md:py-1.5 md:text-sm border border-black rounded-full font-light transition-colors ${selectedSize === size ? 'bg-black text-white' : 'bg-white text-black hover:bg-gray-50'}`} style={{fontFamily: 'Poppins', fontWeight: 300}}>
                      {size}
                    </button>)}
                </div>
              </div>}
